...
#define MODULE_NAME "openconfig-acl"
...
typedef struct {
    enum acl_type type;
    string tag;
    ACL::l2_list::rules_t l2set;
    ACL::l3_list::rules_t l3set;
} acl_set_t;
...
static sr_error_t add_set(acl_set_t &acl_set)
{
    ...
    try {
        ...
        //     OM::mark_n_sweep ms(acl_set.tag+addr);
        //     OM::write(acl_set.tag+addr, l3);
        // }
        if (!acl_set.l3set.empty())
        {
            ACL::l3_list acl(acl_set.tag + "_in", acl_set.l3set);
            OM::write(acl_set.tag + "_in", acl);
        }
    } catch (exception &exc) {
    ...
    foreach_change (ds, it, oper, old_val, new_val) {
        ...
        switch (oper) {
            ...
            case SR_OP_MODIFIED:
                ...
                break;
            case SR_OP_DELETED:
//                rc = parse_acl_del_entry(old_val, &del_mapping);
                break;
            case SR_OP_MOVED:
                break;
        }
